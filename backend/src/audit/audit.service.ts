...
export class AuditService {
  ...
  async record(input: AuditLogInput) {
    try {
      const { error } = await this.supabaseService.client.from('audit_logs').insert({
        ...
        action: input.action,
        module: input.module,
        category: input.category || (input.module === 'auth' || input.module === 'security' ? 'security' : input.actorType === 'user' ? 'workflow' : 'ai_system'),
      ...
      this.logger.warn(`Audit event could not be persisted: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

  async list(query: { search?: string; module?: string; status?: string; category?: string; limit?: number }) {
    ...
    if (query.module && query.module !== 'all') request = request.eq('module', query.module);
    ...
    if (query.search) {
      ...
      request = request.or(`actor.ilike.%${value}%,action.ilike.%${value}%,module.ilike.%${value}%,target.ilike.%${value}%`);
    }
  ...
  async summary() {
    ...
    const automated = events.filter((event) => event.actor_type !== 'user').length;
    const durations = events.map((event) => Number(event.duration_ms)).filter(Number.isFinite);
    return {
      total: events.length,
      pendingApprovals: pending,
      automatedTriggers: automated,
      systemBottlenecks: events.filter((event) => event.status === 'failed').length,
